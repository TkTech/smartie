...
class NVMeStatusFieldError(Exception):
    def __init__(self, status_code: int, status_code_type: int, *, status_filed=None):
        super().__init__()
        self.status_code = status_code
        self.status_code_type = status_code_type
        self.status_filed = status_filed
...
disconnected the Fabric connection).",
        }.get((self.status_code_type, self.status_code), "Unknown Error")

    def __repr__(self):
        return (
            f"<{self.__class__.__name__}(status_code_type=0x{self.status_code_type:02x}, status_code=0x{self.status_code:02x}"
            f" err={self.error_msg!r})>"
        )
